...
UENUM(BlueprintType)
enum class EOATurboModel : uint8
{
	TURBO = 0 UMETA(ToolTip = "Most capable GPT-3.5 model and optimized for chat at 1/10th the cost of . Will be updated with our latest model iteration.text-davinci-003"),
	TURBO_0301 = 1 UMETA(ToolTip = "Snapshot of from March 1st 2023. Unlike , this model will not receive updates, and will only be supported for a three month period ending on June 1st 2023.gpt-3.5-turbogpt-3.5-turbo")
};
...
public:
	...
	TMap<EOATurboModel, FString> turboModels;
};

USTRUCT(BlueprintType)
struct FOAChatMessage
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct FGPT3Settings :public FOASettings
{
	GENERATED_BODY()
